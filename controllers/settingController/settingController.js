...
const updatePassword =  async(req, res) => {
    ...
    try {
        ...
        const checkStmt =await db.sql(`SELECT passwordText FROM policeOfficer WHERE policeOfficerId = '${policeOfficerId}'`);
        try {
            const user = checkStmt;
            if (!user) {
        ...
        const updateStmt =await db.sql(`UPDATE policeOfficer SET passwordText = '${hashedPassword}' WHERE policeOfficerId = '${policeOfficerId}'`);
        try {
...
WHERE 
    po.policeOfficerId = '${policeOfficerId}'`;
        const officer =await db.sql(query);
        if (!officer) {
